...
<div class="branch">
  ...
  <div class="payoff" v-click="4">Пользователь видит таблицу.</div>
</div>
...
[click] Сети у виджета нет по умолчанию.. Сервер в метаданных ресурса перечисляет домены, которые ему нужны, хост из этого списка собирает CSP и вешает на iframe. Список пустой — доступа нет. То есть сервер просит, а разрешает хост.
...
<img
  :src="base + 'images/slides/permission_request.png'"
  alt="Запрос разрешения хоста на вызов тула"
  class="shot"
/>

<style scoped>
/* Скриншот целиком по слайду: вписываем, скругляем, лёгкая рамка и тень */
.shot {
  max-width: 100%;
  max-height: 100%;
  border-radius: 14px;
  border: 1px solid rgba(255, 255, 255, 0.1);
  box-shadow: 0 1.2rem 3.5rem rgba(0, 0, 0, 0.55);
}
</style>

<!--
Так выглядит запрос разрешения: прежде чем виджет вызовет тул, хост показывает пользователю, что именно и с какими аргументами хочет сделать агент. Разрешает — пользователь.
-->
...
<!--
Первая строка бесполезна: виджет без JS. Третья безопасна, но половина фронтенда в ней не заводится — нет storage, нет cookies, любой относительный запрос уходит с opaque origin. Работает вторая, и её же нельзя пускать на страницу хоста: скрипт дотянется до parent.document, снимет атрибут sandbox и перезагрузит себя. Вся хитрость в origin: спека требует, чтобы прослойка жила на другом домене. Там allow-scripts allow-same-origin уже безопасны — снимать sandbox с себя можно сколько угодно, до хоста всё равно не дотянуться. А уже внутри этой прослойки рисуется iframe с HTML приложения.
-->
